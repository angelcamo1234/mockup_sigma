@extends('layouts.app')
@section('title', '- Comparador Mecánica')
...
<div class="form-group px-4">
    <h1 class="mb-3">COMPARADOR MECÁNICA</h1>
            ...
            @foreach ($data as $d)
                <tr>
                    ...
                    @else
                        ...
                        <td></td>
                        <td class="bg-rep-top {{ $d->apply_classes }}">{{ $d->s ? '$' : '' }}{{ number_format($d->d3 * ($d->x100 ? 100 : 1), 0, '', ',') }} {{ $d->porc ? '%' : '' }}</td>
                        <td class="{{ $d->d3p ? 'bg-rep-top ' . $d->apply_classes : '' }} txt-rep-green f-bold">{{ $d->d3p ? round($d->d3p, 0) . '%' : null }}</td>
                        <td></td>
                        <td class="{{ $d->apply_classes }}">{{ $d->s ? '$' : '' }}{{ number_format($d->d4 * ($d->x100 ? 100 : 1), 0, '', ',') }} {{ $d->porc ? '%' : '' }}</td>
